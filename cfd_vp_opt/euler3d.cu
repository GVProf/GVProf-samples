...
__global__ void cuda_compute_step_factor2(int nelr, float *areas, float *step_factors)
{
	const int i = (blockDim.x * blockIdx.x + threadIdx.x);
...
{
	dim3 Dg(nelr / BLOCK_SIZE_2), Db(BLOCK_SIZE_2);
	cuda_compute_step_factor2<<<Dg, Db>>>(nelr, areas, step_factors);
	getLastCudaError("compute_step_factor failed");
}
...
__global__ void cuda_compute_flux2(int nelr, int *elements_surrounding_elements, float *normals, float *fluxes)
{
	const float smoothing_coefficient = float(0.2f);
	const int i = (blockDim.x * blockIdx.x + threadIdx.x);
...
{
	dim3 Dg(nelr / BLOCK_SIZE_3), Db(BLOCK_SIZE_3);
	cuda_compute_flux2<<<Dg, Db>>>(nelr, elements_surrounding_elements, normals, fluxes);
	getLastCudaError("compute_flux failed");
}
...
__global__ void cuda_time_step2(int j, int nelr, float *variables, float *step_factors, float *fluxes)
{
	const int i = (blockDim.x * blockIdx.x + threadIdx.x);
...
{
	dim3 Dg(nelr / BLOCK_SIZE_4), Db(BLOCK_SIZE_4);
	cuda_time_step2<<<Dg, Db>>>(j, nelr, variables, step_factors, fluxes);
	getLastCudaError("update failed");
}
