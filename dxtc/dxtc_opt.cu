...
static __device__ float evalPermutation3_opt(const float3 *colors, uint permutation, ushort *start, ushort *end, float3 color_sum, float3 alphax_sum, int akku)
{
#if USE_TABLES
    float alpha2_sum = float(akku >> 16);
    float beta2_sum = float((akku >> 8) & 0xff);
...
__device__ void evalAllPermutations(const float3 *colors, const uint *permutations, ushort &bestStart, ushort &bestEnd, uint &bestPermutation, float *errors, float3 color_sum, cg::thread_block cta)
{
    ...
    for (int i = 0; i < 3; ++i) {
        alphax_sum[i] = make_float3(0.0f, 0.0f, 0.0f);
        akku[i] = 0;
    }
    ...
    for (int i = 0; i < 16; i++) {
      ...
      for (int j = 0; j < 3; j++) {
        ...
        uint permutation = 0;
        if (i == 0) {
          permutation3[j] = s_permutations[pidx];
        }
        permutation = permutation3[j];

        const uint bits = permutation >> (2*i);

        alphax_sum[j] += alphaTable3[bits & 3] * color;
        akku[j] += prods3[bits & 3];
      }
    }
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        ushort start, end;
        uint permutation = permutation3[i];
        float error = evalPermutation3_opt(colors, permutation, &start, &end, color_sum, alphax_sum[i], akku[i]);
