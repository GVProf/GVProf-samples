...
__global__ void srad2(	fp d_lambda, 
	...
	if(ei<d_Ne){															// make sure that only threads matching jobs run
		...
		d_cN=1.0;
		d_cS = 1.0;								// south diffusion coefficient
		d_cW = 1.0;														// west diffusion coefficient
		d_cE = 1.0;

		// if (d_cN - 1 > 10e-4 or d_cS - 1 > 10e-4 or d_cW - 1 > 10e-4 or d_cE - 1 > 10e-4)
		// {
		// 	printf("%s\n", "in srad2 there's value not in 0,1");
		// 	/* code */
		// }
