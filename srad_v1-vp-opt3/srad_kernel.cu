...
__global__ void srad(fp d_lambda, int d_Nr, int d_Nc, long d_Ne, int *d_iN,
                     ...
                     fp *d_dE, fp *d_dW, fp d_q0sqr, uint32_t *d_c, fp *d_I) {
...
unsigned mask = __ballot_sync(FULL_MASK, ei < d_Ne);
  ...
  asm volatile("mov.u32 %0, %%laneid;" : "=r"(laneid));
        ...
        if (d_c_loc < 0) {        // if diffusion coefficient < 0
            d_c_loc_2 = 0;          // ... set to 0
        } else if (d_c_loc > 1) { // if diffusion coefficient > 1
            d_c_loc_2 = 1<<laneid;          // ... set to 1
        }
        for (int offset = warpSize/2; offset > 0; offset = offset <<1) 
            d_c_loc_2 += __shfl_down_sync(mask, d_c_loc_2, offset);
