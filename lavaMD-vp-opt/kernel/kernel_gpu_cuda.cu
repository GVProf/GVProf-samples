								...
								FOUR_VECTOR* d_fv_gpu)
{
	...
	// while(wbx<box_indexes_counter){
      ...
      rA_shared[wtx].v = rA[wtx].v / 10.0;
      rA_shared[wtx].x = rA[wtx].x / 10.0;
      rA_shared[wtx].y = rA[wtx].y / 10.0;
      rA_shared[wtx].z = rA[wtx].z / 10.0;
		...
		for (k=0; k<(1+d_box_gpu[bx].nn); k++){
			...
			while(wtx<NUMBER_PAR_PER_BOX){
        rB_shared[wtx].v = rB[wtx].v / 10.0;
        rB_shared[wtx].x = rB[wtx].x / 10.0;
        rB_shared[wtx].y = rB[wtx].y / 10.0;
        rB_shared[wtx].z = rB[wtx].z / 10.0;
				qB_shared[wtx] = qB[wtx] / 10.0;
				wtx = wtx + NUMBER_THREADS;
			}
			wtx = tx;
