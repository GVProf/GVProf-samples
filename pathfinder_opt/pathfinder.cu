...
void init(int argc, char **argv) {
  ...
  wall = new int8_t *[rows];

  for (int n = 0; n < rows; n++)
    wall[n] = new int8_t[cols];
...
__global__ void dynproc_kernel(int iteration, int8_t *gpuWall, int *gpuSrc,
...
*/
int calc_path(int8_t *gpuWall, int *gpuResult[2], int rows, int cols,
...
void run(int argc, char **argv) {
  ...
  cudaMalloc((void **)&gpuWall, sizeof(int8_t) * (size - cols));
  cudaMemcpy(gpuWall, wall, sizeof(int8_t) * (size - cols), cudaMemcpyHostToDevice);
